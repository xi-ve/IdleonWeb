...
class VaultUnlockerPlugin(PluginBase):
    ...
    async def get_cached_vault_list(self):
        ...
        if (not hasattr(self, '_vault_cache') or 
            ...
            try:
                ...
                if not raw_result:
                    console.print("[vault_unlocker] No raw result from JS")
                    return []
    ...
    async def get_vault_autocomplete(self, query: str = ""):
        console.print(f"[vault_unlocker] get_vault_autocomplete called with query: '{query}'")
        try:
            ...
            console.print(f"[vault_unlocker] Returning {len(suggestions)} suggestions")
            return suggestions
        except Exception as e:
            console.print(f"[vault_unlocker] Error in get_vault_autocomplete: {e}")
            return []

    async def get_set_vault_item_level_autocomplete(self, query: str = ""):
        return await self.get_vault_autocomplete(query)
    ...
    def get_vault_item_names_js(self):
        ...
        try {
            ...
            if (!upgVault || !upgradeVault) {
                console.log("[vault_unlocker] Error: Vault data not found");
                return "Error: Vault data not found";
            }
            
            console.log("[vault_unlocker] Processing vault categories for autocomplete");
            ...
            for (let i = 0; i < upgradeVault.length; i++) {
                ...
                console.log("[vault_unlocker] Found category: " + categoryName + " (max: " + maxLevel + ")");
            }
            
            console.log("[vault_unlocker] Returning vault categories for autocomplete");
            return output;
        } catch (e) {
            console.log("[vault_unlocker] Error in get_vault_item_names_js:", e);
            return "Error: " + e.message;
        }
        '''
    ...
    @js_export()
    def get_vault_autocomplete_js(self):
        return '''
        try {
            const ctx = window.__idleon_cheats__;
            if (!ctx?.["com.stencyl.Engine"]?.engine) throw new Error("Game engine not found");
            ...
            if (!upgradeVault) {
                console.log("[vault_unlocker] No upgradeVault data found");
                return [];
            }
            ...
            for (let i = 0; i < upgradeVault.length; i++) {
                ...
                suggestions.push(`${categoryName} 1`);
                suggestions.push(`${categoryName} ${maxLevel}`);
                suggestions.push(`${categoryName} ${Math.floor(maxLevel/2)}`);
            }
            
            return [...new Set(suggestions)];
        } catch (e) {
            console.log(`[vault_unlocker] Error in get_vault_autocomplete_js: ${e.message}`);
            return [];
        }
        '''
