...
def collect_plugin_js(plugin_manager):
    ...
    for plugin in plugins:
        ...
        js_size = len(js_code) / len(plugins) if plugins else 0  # Rough estimate
        table.add_row(name, str(func_count), f"{js_size/1024:.2f}")
    
    console.print(table)

def update_inject_files():
    """Update inject files list in config."""
...
def cmd_inject(args=None, plugin_manager=None):
    """Run the injector with current config."""
...
class HierarchicalCompleter:
    ...
    def get_completions(self, document, complete_event):
        ...
        # Get command names that start with the last token
    ...
    async def get_completions_async(self, document, complete_event):
        """Async version of get_completions for newer prompt_toolkit versions."""
        text = document.text_before_cursor.strip()
        last_token = text.split()[-1] if text else ''
